...
export default function NetDiskSearchResults({ results, loading, error, total }: NetDiskSearchResultsProps) {
  ...
  const [playingLinks, setPlayingLinks] = useState<{ [key: string]: boolean }>({});
  ...
  const handlePlay = async (shareUrl: string, sharePwd: string, linkKey: string, platform: string) => {
    ...
    setPlayingLinks(prev => ({ ...prev, [linkKey]: true }));
    ...
    try {
      const response = await fetch('/api/netdisk/play', {
        ...
        body: JSON.stringify({
          shareUrl,
          sharePwd: sharePwd || undefined,
        }),
      });

      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || '获取播放地址失败');
      }

      const data = await response.json();

      // 跳转到播放页面
      router.push(`/play?url=${encodeURIComponent(data.playUrl)}&title=${encodeURIComponent(data.fileName || '网盘视频')}`);
    } catch (err) {
      console.error('播放失败:', err);
      alert(err instanceof Error ? err.message : '播放失败，请稍后重试');
    } finally {
      setPlayingLinks(prev => ({ ...prev, [linkKey]: false }));
    }
  };
                      ...
                      <div className="sm:ml-4 flex-shrink-0 flex flex-col sm:flex-row gap-2">
                        ...
                        {(type === 'aliyun' || type === 'pikpak' || type === '123' || type === '115') && (
                          <button
                            ...
                            className="inline-flex items-center px-3 py-2 sm:py-1 border border-blue-500 dark:border-blue-400 rounded-md text-xs font-medium text-white bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-700 transition-colors w-full sm:w-auto justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {playingLinks[linkKey] ? (
                              <>
