...
/**
 * 检测 M3U8 流是否支持 DVR/时移回放功能
 * @param m3u8Url M3U8 播放地址
 * @param ua User-Agent
 * @returns 是否支持 DVR
 */
export async function detectDVRSupport(m3u8Url: string, ua?: string): Promise<boolean> {
  try {
    ...
    // 检查是否包含 EXT-X-PROGRAM-DATE-TIME（时间戳标记，DVR的关键标识）
    const hasProgramDateTime = text.includes('#EXT-X-PROGRAM-DATE-TIME');

    // 检查是否是已结束的VOD（有ENDLIST表示已结束）
    const hasEndList = text.includes('#EXT-X-ENDLIST');

    // 检查是否是EVENT类型（可能支持时移）
    const isEvent = text.includes('#EXT-X-PLAYLIST-TYPE:EVENT');

    // 检查目标时长，DVR通常有较长的片段列表
    const targetDurationMatch = text.match(/#EXT-X-TARGETDURATION:(\d+)/);
    const segments = (text.match(/#EXTINF:/g) || []).length;

    // 计算大致的播放列表时长（秒）
    let playlistDuration = 0;
    if (targetDurationMatch && segments > 0) {
      const targetDuration = parseInt(targetDurationMatch[1]);
      playlistDuration = targetDuration * segments;
    }

    // DVR判断逻辑：
    // 1. 有时间戳标记 + 不是已结束的VOD = 可能支持DVR
    // 2. 播放列表时长超过5分钟（300秒）也可能支持时移
    const likelyDVR = hasProgramDateTime && (!hasEndList || isEvent);
    const longPlaylist = playlistDuration > 300;

    return likelyDVR || longPlaylist;
  } catch (error) {
    console.error('检测DVR支持失败:', error);
    return false;
  }
}