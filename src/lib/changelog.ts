...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: "2025-08-28",
    added: [
      "新增弹幕字号动态调整功能，支持小号(18px)、默认(25px)、大号(32px)、特大(40px)四种规格",
      "新增弹幕显示区域动态调整功能，支持1/4屏幕、1/2屏幕、3/4屏幕、全屏四种显示范围",
      "新增弹幕配置错误处理机制，防止字号和区域调整时播放器崩溃"
    ],
    changed: [
      "优化外部弹幕开关性能，使用Promise.resolve()替代setTimeout()实现更好的异步处理",
      "改进弹幕显示区域配置，使用百分比值(25%)替代固定像素值提升不同屏幕适配性",
      "增强状态同步机制，防止React状态更新和ArtPlayer插件状态不一致"
    ],
    fixed: [
      "修复外部弹幕开关按钮点击延迟和无响应问题",
      "修复关闭外部弹幕后仍显示弹幕的问题，现在会完全清空弹幕数据",
      "修复快速切换外部弹幕开关时的竞态条件问题",
