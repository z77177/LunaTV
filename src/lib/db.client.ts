...
export async function savePlayRecord(
  ...
  try {
    ...
    // 🔑 方案A：访客模式下，虽然数据存在本地，但异步上报一份到服务器用于汇总统计
    if (SYSTEM_STORAGE_TYPE !== 'localstorage') {
      try {
        const auth = getAuthInfoFromBrowserCookie();
        if (auth?.isGuest) {
          // 异步上报，不等待结果
          fetchFromApi('/api/playrecords', {
            ...
            body: JSON.stringify({ key, record }),
          }).catch(e => console.warn('访客统计上报失败:', e));
        }
      } catch (e) {
        // ignore
      }
