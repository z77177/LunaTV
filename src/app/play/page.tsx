      ...
      artPlayerRef.current.on('ready', async () => {
        ...
        const addMobileDanmakuOptimization = () => {
          ...
          if (shouldOptimizeForMobile) {
            ...
            // JavaScript交互优化 - 完全参考alpha版本的实现（添加重试机制）
            const addMobileDanmakuToggle = () => {
              const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

              // 增加重试机制，确保弹幕插件完全加载 - alpha版本的关键！
              let retryCount = 0;
              const maxRetries = 10;
              ...
              const tryAddToggleButton = () => {
                ...
                if (!danmakuContainer || !configButton || !configPanel) {
                  retryCount++;
                ...
                console.log('找到弹幕配置按钮，开始设置移动端交互 - alpha版本策略');
                ...
                console.log('🔧 移动端弹幕交互优化完成 - 完全参考alpha版本实现（含重试机制）');
              };

              // 开始尝试添加按钮 - alpha版本的重试逻辑
              tryAddToggleButton();
            };
