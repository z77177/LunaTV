        ...
        const addMobileDanmakuToggle = () => {
          ...
          setTimeout(() => {
            ...
            if (isMobile) {
              ...
              // 🖥️ 桌面端：基于官方源码的智能hover优化
              console.log('桌面端启用智能hover优化（基于官方源码）');
              ...
              // 🧠 官方onMouseEnter增强版 - 添加延迟和意图检测
              const enhancedMouseEnter = (control: Element, panel: Element, panelType: 'config' | 'style') => {
                const currentTime = Date.now();
                const timeSinceLastEnter = currentTime - lastMouseEnterTime;
                lastMouseEnterTime = currentTime;
                
                // 如果鼠标快速连续进入（可能是误触），添加延迟
                const isQuickMove = timeSinceLastEnter < 100;
                const delay = isQuickMove ? 300 : 150; // 快速移动时延迟300ms，正常150ms
                
                const timer = panelType === 'config' ? configHoverTimer : styleHoverTimer;
                ...
                const newTimer = setTimeout(() => {
                  ...
                  if (control.matches(':hover')) {
                    ...
                    if (left > 0) {
                      panelEl.style.left = `${-half + left}px`;
                    } else if (right > 0) {
                      panelEl.style.left = `${-half - right}px`;
                    } else {
                      panelEl.style.left = `${-half}px`;
                    }
                    ...
                    console.log(`🎯 智能hover显示${panelType}面板 (延迟:${delay}ms)`);
                  }
                }, delay);
                
                if (panelType === 'config') {
                  configHoverTimer = newTimer;
                } else {
                  styleHoverTimer = newTimer;
                }
              };
              
              // 🏃 智能mouseleave处理
              const enhancedMouseLeave = (control: Element, panel: Element, panelType: 'config' | 'style') => {
                // 给用户100ms的缓冲时间移动到面板上
                setTimeout(() => {
                  if (!control.matches(':hover') && !panel.matches(':hover')) {
                    const panelEl = panel as HTMLElement;
                    ...
                    console.log(`❌ 智能hover隐藏${panelType}面板`);
                  }
                }, 100);
              };
              
              // 🚫 先禁用原版CSS hover，让JS接管
              const disableCSSHover = document.createElement('style');
              disableCSSHover.textContent = `
                /* 禁用原版CSS hover */
              ...
              `;
              document.head.appendChild(disableCSSHover);
              
              // 🎯 绑定增强的hover事件
              configButton.addEventListener('mouseenter', () => {
                enhancedMouseEnter(configButton, configPanel, 'config');
              });
              
              configButton.addEventListener('mouseleave', () => {
                enhancedMouseLeave(configButton, configPanel, 'config');
              });
              ...
              if (styleButton && stylePanel) {
                styleButton.addEventListener('mouseenter', () => {
                  enhancedMouseEnter(styleButton, stylePanel, 'style');
                });
                
                styleButton.addEventListener('mouseleave', () => {
                  enhancedMouseLeave(styleButton, stylePanel, 'style');
                });
              ...
              console.log('✅ 桌面端智能hover优化已启用（基于官方源码）');
            }
