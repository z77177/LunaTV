...
function LivePageClient() {
  ...
  const [livePlaybackMode, setLivePlaybackMode] = useState<'auto' | 'live' | 'dvr'>('auto');
  const [supportsDVR, setSupportsDVR] = useState(false);

  // 从 localStorage 加载播放模式设置
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('livePlaybackMode');
      if (saved && (saved === 'auto' || saved === 'live' || saved === 'dvr')) {
        setLivePlaybackMode(saved as 'auto' | 'live' | 'dvr');
      }
    }
  }, []);
