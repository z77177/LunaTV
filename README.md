...
本项目是在 **MoonTV** 基础上进行的深度二次开发版本，从 **v4.3.1** 版本开始，持续迭代至当前 **v5.9.0**，累计新增 60+ 重大功能模块，400+ 细节优化。所有新增功能详见 [CHANGELOG](CHANGELOG)。
...
### 最新版本：v5.9.0 (2025-12-27)

#### 新增功能
- 🔐 多Provider OIDC支持：同时配置多个OAuth提供商（Google、Microsoft、GitHub、Facebook、微信、Apple、LinuxDo）
- 🔐 GitHub OAuth完整适配：自动使用正确scope、Accept header、私有邮箱获取
- 🍎 Apple Sign In支持：form_post响应模式、id_token解析、JWKS签名验证
- 👥 多人观影房功能：外部服务器集成的实时同步观影，支持房间管理、播放同步、成员状态显示
- 📥 M3U8下载功能：客户端M3U8视频下载支持，批量剧集下载
- 🎬 播放器缓冲优化：三种缓冲模式（省流、均衡、高质），智能适配网络环境
- 🎨 Anime4K超分辨率：WebGPU加速的实时视频超分辨率，提升画质
- 🎯 自定义广告过滤：支持自定义广告过滤规则代码，独立重置和恢复默认按钮
...
#### 性能优化
- 🔐 V2用户存储系统：SHA256加密，改进的用户管理和OIDC集成
- 👥 观影房全局状态管理：重构连接和状态管理，优化剧集显示和用户名加载
- 📊 收藏同步优化：优化超时处理，移除过多的错误提示
- 🍪 Cookie加载检测：改进持久轮询检测机制

#### Bug 修复
- 🔐 修复OIDC redirect_uri：支持本地开发（0.0.0.0→localhost）和反向代理环境
- 🔐 修复GitHub OAuth集成：正确的scope、Accept header、id_token验证排除
- 🔐 修复Apple OIDC发现：改进Apple OAuth配置和POST handler
- 🔐 修复OIDC注册和用户同步：确保新用户正确创建和显示在admin面板
- 👥 修复观影房多项问题：用户名硬编码、剧集切换同步、首次访问401、SSR默认guest等35+修复
- 📥 修复Windows上的M3U8下载和husky：改进Windows兼容性
- 🖼️ 修复图片预加载querySelector错误：处理格式错误的URL
